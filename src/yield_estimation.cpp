...
Int_t main(int argc, char** argv) {
    ...
    if (path.Contains(reaction1)) {
        spline_root_file_path = Form("%s/data/spline/etaLambda_CB_spline.root", WORK_DIR.Data());
    } else if (path.Contains(reaction2)) {
        spline_root_file_path = Form("%s/data/spline/Kp_bubble1970_spline.root", WORK_DIR.Data());
    } else if (path.Contains(reaction3)) {
        spline_root_file_path = Form("%s/data/spline/K0n_bubble1970_spline.root", WORK_DIR.Data());
    } else if (path.Contains(reaction4)) {
        spline_root_file_path = Form("%s/data/spline/piPlusSigmaMinus_bubble1970_spline.root", WORK_DIR.Data());
    } else if (path.Contains(reaction5)) {
        spline_root_file_path = Form("%s/data/spline/piMinusSigmaPlus_bubble1970_spline.root", WORK_DIR.Data());
    } else if (path.Contains(reaction6)) {
        spline_root_file_path = Form("%s/data/spline/pi0Sigma0_bubble1970_spline.root", WORK_DIR.Data());
    } else if (path.Contains(reaction7)) {
    ...
    for (Double_t i = 0; i < conf.n_mom_points; i++) {
        ...
        // -- calc n react -----
        Double_t tmp_n_react = cs*TMath::Power(10.0, -27.0) * (*n_kaon_container)[ana_helper::get_index(mom)] * conf.density_LH2 * conf.Na * conf.d;
