...
def plot(arg_dict):
    ...
    edep_list = [
        ...
        [f"{prefix}edep_proton",   "C0", "proton"],
    ...
    for keys in edep_list:
        ...
        ax1.hist(center, bins=edge, weights=value, lw = 2, histtype='step', color=keys[1], label = keys[2])
    ...
    # -- hit pattern -----
    ax2 = ax[1]
    values, edges_x, edges_y = file[f"{prefix}hitpat_vs_edep_proton"].to_numpy()  # ヒストグラムのデータを取得
