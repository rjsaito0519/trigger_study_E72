    ...
    for ax, ax_sub, arg_dict in zip([ax1, ax2, ax3], [ax4, ax5, ax6], [arg_dict1, arg_dict2, arg_dict3]):
        ...
        for rawdata, key in zip(arg_dict["data"], arg_dict["data_key"]):
            ...
            ax.errorbar(
                ...
                yerr = a0_err, 
                fmt = "o", capsize = 0, markeredgecolor = "k", ms = 0, ecolor='k', color = "w", zorder = 5, elinewidth = 1.5, label = "E72"
            )
...
def legend_plot():
    ...
    fig_legend.legend(
        handles=legends,
        labels=["bubble chamber (1970)", "bubble chamber (1968)", "bubble chamber (1975)", "Crystal Ball", "counter", "spark chamber", "E72"],   # ラベル（凡例テキスト）
