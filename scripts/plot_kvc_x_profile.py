...
for mom in [645, 735, 805]:
    ...
    ax.errorbar(
        tree["z_pos"][idx],
        tree["stdev_val"][idx],
        yerr = tree["stdev_err"][idx],
        fmt = "o", capsize = 0, markeredgecolor = "k", ms = 8, 
        ecolor='k', color=f'C{i}', markeredgewidth = 0.1, zorder = 3,
        label = r"$p_K = ${} MeV/c".format(mom)
    )
    model = lfm.LinearModel()
    params = model.guess(x = tree["z_pos"][idx], data = tree["stdev_val"][idx])
    result = model.fit(x = tree["z_pos"][idx], data = tree["stdev_val"][idx], weights = 1.0/tree["stdev_err"][idx], params=params, method='leastsq')
    print(mom)
    print(result.fit_report())
    print("------------------")
    fit_x = np.linspace(min(tree["z_pos"][idx]), max(tree["z_pos"][idx]), 10)
...
plt.subplots_adjust(left = 0.15, right=0.98, top=0.98, bottom = 0.12)
img_save_path = os.path.join(script_dir, "../results/img/kvc_beam_stdev.png")
os.makedirs(os.path.dirname(img_save_path), exist_ok=True)
plt.savefig(img_save_path, format='png', bbox_inches='tight', dpi=600, transparent=True)
plt.show()